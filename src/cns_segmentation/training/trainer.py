...
class SegmentationTrainer:
    ...
    def setup_data(self) -> None:
        ...
        # Resolve dataset registry entry/entries; default matches the
        # pre-registry single-cord-label behavior exactly (label_keys=None).
        # `data.dataset` may be a single registry key (legacy) or a list of
        # registry keys (multi-structure) — the latter drives compositing.
        ...
        if multi_structure:
            ...
            non_train = [s for s in specs if s.role != "train"]
            if non_train:
                offenders = ", ".join(f"{s.name} (role={s.role})" for s in non_train)
                raise ValueError(
                    f"Cannot train on dataset(s) not marked role='train': {offenders}. "
                    f"Use a role='train' dataset, or update the registry entry if this "
                    f"is intentional."
                )

            allow_mixed_regions = data_cfg.get("allow_mixed_regions", False)
            regions = {s.spinal_region for s in specs}
            if len(regions) > 1 and not allow_mixed_regions:
                raise ValueError(
                    f"Datasets {dataset_names} span multiple spinal regions "
                    f"{sorted(regions)}. Mixing regions in one training run "
                    f"changes what the model is being asked to generalize over — "
                    f"set data.allow_mixed_regions: true to opt in explicitly."
                )

            label_keys = merge_label_keys(*specs)
            self.structures = [s for s in DEFAULT_LABEL_PRIORITY if s in label_keys]
            # Full supervision only this phase (see ROADMAP Phase 1 Decision
            # 1): every requested structure must be present on a subject, or
            # it's dropped rather than trained with a silently-zero channel.
            require_all_labels = len(self.structures) > 1
        ...
        if multi_structure:
            train_files = flatten_structure_labels(train_files)
            val_files = flatten_structure_labels(val_files)
        ...
        if multi_structure:
            expected_out_channels = len(self.structures) + 1
            configured_out_channels = self.config["model"].get("out_channels", 2)
            if configured_out_channels != expected_out_channels:
                raise ValueError(
                    f"model.out_channels={configured_out_channels} does not match "
                    f"structures {self.structures} (expected {expected_out_channels} "
                    f"= 1 background + {len(self.structures)} structures)"
                )
        train_transforms = get_train_transforms(transform_config, structures=self.structures)
        val_transforms = get_val_transforms(transform_config, structures=self.structures)
    ...
    def setup_model(self) -> None:
        ...
        if loss_name == "DiceCELoss":
            ...
            self.loss_function = CombinedLoss(**loss_params)
        else:
